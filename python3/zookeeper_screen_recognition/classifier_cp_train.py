...
INPUT_WIDTH  = classifier_cp_model.INPUT_WIDTH
INPUT_HEIGHT = classifier_cp_model.INPUT_HEIGHT
...
def load_img_list_csv(v_dict_list):
    ...
    ret = np.array(ret)
    assert(ret.shape==(len(v_dict_list),INPUT_HEIGHT,INPUT_WIDTH,3))
    return ret
...
if __name__ == '__main__':
    ...
    out_dir = os.path.join('model','cp')

    csv_path = os.path.join('label','cp.csv')
    sample_list = _util.read_csv(csv_path,add_cp.COL_NAME_LIST)
    ...
    model = classifier_cp_model.create_model(label_count)
    model.summary()
