...
INPUT_WIDTH  = classifier_battle_second_model.INPUT_WIDTH
INPUT_HEIGHT = classifier_battle_second_model.INPUT_HEIGHT
...
def load_img_fn(fn):
    ...
    img = classifier_battle_second_model.preprocess_img(img)
    return img
...
if __name__ == '__main__':
    ...
    out_dir = os.path.join('model','battle_second')

    csv_path = os.path.join('label','battle_second.csv')
    sample_list = _util.read_csv(csv_path,add_battle_second.COL_NAME_LIST)
    ...
    model = classifier_battle_second_model.create_model(label_count)
    model.summary()
