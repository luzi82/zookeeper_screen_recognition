...
def create_model(label_count):
    ...
    model.add(Conv2D(filters=6, kernel_size=4, padding='valid', activation='elu'))
    model.add(Conv2D(filters=6, kernel_size=1, padding='valid', activation='elu'))
    model.add(Conv2D(filters=8, kernel_size=4, padding='valid', activation='elu'))
    model.add(Conv2D(filters=8, kernel_size=1, padding='valid', activation='elu'))
    model.add(Conv2D(filters=10, kernel_size=(1,6), padding='valid', activation='elu'))
    model.add(Conv2D(filters=10, kernel_size=1, padding='valid', activation='elu'))
    model.add(Flatten())
